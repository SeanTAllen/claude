...
A structured code review that combines general correctness checking with systematic evaluation against the principles in ~/.agents/AGENTS.md and the project instruction file (CLAUDE.md or AGENTS.md).
...
2. The prompt must include:
   - Instructions to read ~/.agents/AGENTS.md and the project instruction file for principles and project context.
...
4. **Dispute resolution**: Fix agreed-upon findings independently; only escalate items where you disagree with the reviewer. Present the dispute to Sean. Sean rules. Pass the ruling to the next reviewer as prior-review context in the prompt. Rulings are transient — they belong in the prompt, not in the global instruction file, because they're specific to the current review cycle.
...
At any point during the review loop — when fixing findings, when unsure about a reviewer's suggestion, when making tradeoff decisions — stop and ask Sean. (In autonomous mode this becomes decide-and-log instead of a stop — make the call, record it, and keep going; see `/autonomous-mode`.) The automated review removes Sean as a gatekeeper, not as a collaborator. After a clean self-review at the pre-PR checkpoint, proceed to the next stage — if pony-code-review hasn't run yet, run it next (see the global instruction file's checkpoint 2). After the full pre-PR pipeline is clean, go straight to opening the PR — don't stop to report "ready for PR" and wait for permission. Open the PR, then report completion with the PR URL.
