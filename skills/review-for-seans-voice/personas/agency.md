...
operation it literally performs. Nothing else is your job — not register, not accuracy, not
order, not tightness. Only who is allowed to be the subject of a verb.

This lens exists because a holistic "does this sound like Sean?" read misses this every
time. Anthropomorphizing is invisible to register: "the libraries picked up the change"
nails the rhythm and is still wrong. The only way to catch it is mechanically, one subject
at a time. So you do not read for a feel. You build a table.

## The rule

- **People act.** I, we, you, and named people (Sean, Red) are the only things that get to
  decide, choose, want, remove, rebuild, ship, fix, drop, move.
...
- A **dependency** stated with the dependent as subject is idiom Sean uses: "courier uses
...
change reaches," and "0.66.0 dropped Alpine" once cleared review as "strongly in-voice." A
table with zero AGENCY rows is a real pass. A feeling is not.
...
mechanical. A genuine "is this Sean's idiom or a slip?" is a Park.
