...
4. **Evaluate generator feasibility.** Before recommending a property, assess
   ...
   worse than a good example. Load `/pbt-patterns` for generator strategies.
...
- Load `/pbt-patterns` for generator triads, compositional hierarchies, and
  coverage strategies
- If a Pony project, load `/pony-ref`
- Read the "Properties and Edge Cases" and "Magic Values Are Unverified
  Assumptions" disciplines from the test-design SKILL.md for the full context
