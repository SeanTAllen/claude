...
A single agent applying the test-design disciplines will still pattern-match —
...
problem. The dominance heuristic from `/synthesize` ("when one agent's output
...
**Skipping evaluation.** When running the test-design ensemble, planning tests
