---
name: reap-memories
description: Review the memory inbox — the plain fallback captures written to ~/.agents/memory-inbox during sessions — and decide with Sean which few are worth promoting into the global instruction file or a skill and which to discard, opening a PR per promotion. Load when Sean runs /reap-memories.
...
- **Location**: `~/.agents/memory-inbox/<repo>/<file>.md` — one file per captured memory. Each file's parent directory is the repository the capture came from, so that origin never needs to be written inside the file; it's how you route the candidate.
...
2. **Gather.** Read every `~/.agents/memory-inbox/*/*.md`. If the inbox is missing or empty, there is nothing to reap — say so and stop. Present the memories to Sean grouped by origin repository.
...
- **A general working habit, preference, or correction of Sean's** → his global AGENTS.md, or a new/existing personal skill → **seantallen/llm-skills**.
...
- **A fact specific to one project** → that project's instruction file (`CLAUDE.md` or `AGENTS.md`). Use `pony-agents-md` for what belongs there versus what doesn't.
- **Something substantial and standalone** → a new skill in whichever repo owns it.

`pony-agents-md` is language-agnostic and governs the project instruction file decision for any project. `pony-skills` is Pony-specific and only matters for Pony-destined promotions. Both are installed in `~/.agents/skills`, so they load in any session; this skill assumes they're present. If the pony index is ever absent, the global-versus-project routing above still works through the language-agnostic `pony-agents-md`.
