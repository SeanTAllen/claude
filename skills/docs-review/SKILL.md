...
Ensemble documentation review that synthesizes findings from specialized reviewer personas. Has two modes: full (8 personas, iterative re-review) and lightweight (3 personas, single pass). Not for one-line typo fixes or trivial formatting changes — ask permission to skip review for those. The principle-review self-review (checkpoint 2, stage 1) is sufficient for trivial documentation changes.
...
3. **Create a temporary directory for evidence files.** Use `~/tmp/docs-review-<timestamp>/`. Each persona will write its detailed evidence to a file in this directory. Generate the file path for each persona (e.g., `accuracy-evidence.md`) and pass it in the prompt.
...
4. **Spawn 8 persona agents in parallel** as Tasks with subagent_type="general-purpose" and model="opus". Each agent's prompt includes:
   ...
   - The persona document, read from the corresponding file in `personas/`. When a persona's context loading references an external skill (e.g., `/pony-ref`, `/seans-voice`), read that skill's content and include it in the agent prompt.
   ...
   - Relevant gathered context from step 2 (source code for Accuracy, related docs for Consistency, style guides for Principles).
...
When docs-review runs as part of the pre-PR pipeline, findings go back to the implementer for triage and fixing. The loop repeats until clean.
...
- **Fix**: The right action is obvious from the finding itself. Factual errors, broken examples, missing steps, stale cross-references, terminology inconsistencies. Fix without waiting for Sean.
- **Park**: The finding needs Sean's input. Tone questions, scope decisions, audience disagreements, ambiguous tradeoffs where reasonable people could disagree. Also park findings you disagree with — don't dismiss them. Parked items are listed in the PR for Sean to weigh in on.
...
After fixing the "fix" items, docs-review runs again with two key rules:
...
3. **Create a temporary directory for evidence files.** Use `~/tmp/docs-review-<timestamp>/`. Same convention as full mode.
...
| `principles.md` | Systematic check against documentation conventions and CLAUDE.md principles |
