...
- **A mistake you already fixed is not a load-bearing fact.** When you get something wrong and correct it before it reaches Sean — a draft you rewrote, a subagent's bogus output you caught, a wrong approach you abandoned — it does not go in the report. Report the residue, not the error: if what he's getting is still wrong, still unverified, or was narrowed to get it done, say that, because that is what he's relying on. If nothing he holds is untrue, the accounting is noise. This is silence in summaries, not denial under question — if he asks what happened, tell him. And if the mistake exposes a defect in a tool or skill he owns — a review agent that fabricates its findings, say — tell him that, because it's a bug in his tooling, not because it was your error.
...
**Diagnose before you plan**: When work starts from a bug report or an issue, the diagnosis comes first — reproduce, find the cause, find how far it reaches, find where the fix belongs — and the plan is built from that, not from the report. Load `/pony-debug` and work it through before writing the plan. A plan written from the report has already fixed the scope at the reported symptom, and every check after that is arguing against a plan Sean already approved.
...
**Research findings belong in the plan**: If research or exploration surfaces issues beyond the original task (inaccurate comments, dead code, related bugs), include them as explicit plan steps — don't just mention them in the analysis and move on. Anything worth noting is worth acting on or explicitly deferring. A finding is only outside the current branch's scope once you've asked whether it means that scope is too small; what's left over gets captured as a suspected issue to vet later, rather than filed on the spot — see `/vet-suspected-issues`.

**Vet suspected issues before filing them**: A problem you spot while working on something else is not filed as an issue on the spot — and it is not automatically out of scope either. First ask whether it means the work you're doing is scoped too small. When the work is a bug, that means asking whether one cause could produce both it and what you're fixing; when it's a feature, whether the thing you're building should already cover it. If you can state a way it belongs, go look before you decide. Only what doesn't belong becomes a suspected issue: capture it, and after the main work's PR is open, vet it — verify, scope, review — then file a correct issue or discard it. Load `/vet-suspected-issues` for the protocol.
...
**Writing is design work, not implementation**: The audience, the narrative, and what goes in or stays out are choices with no single right answer, and the criteria live partly in Sean's head. So writing gets what a design gets — show him the shape before the words exist, and expect to iterate on it. The narrative outline is a design document, not a spec. A review of a finished draft is not a substitute: it checks the prose against criteria you already chose, so it comes back clean on a piece built on the wrong shape.

**How to write anything**: Any writing on Sean's behalf runs the `/writing-handoff` process — comments, docstrings, commit messages, PR descriptions, issues, discussions, blog posts, LWIP, all of it. Load `/writing-handoff` first. It exists because the agent holding the full context writes unclear prose without noticing — it fills the gaps from memory a reader doesn't have (the curse of knowledge) — so it splits the job: the context-holding agent designs the piece with Sean, a fresh agent writes the words, and the context-holding agent checks the result for fidelity. It scales: the full two-agent handoff for substantial prose, a fresh-reader pass on your own draft for short forms like a one-line commit or a docstring. It dispatches the fresh writer to the rulebook for the form. For voice forms (blog posts, LWIP, discussions, issues, PR descriptions, commit messages, announcements): `/how-to-write` for the process and `/seans-voice` for the sound, plus the form skill where there is one (`/blog-post`, `/sean-lwip`), and `/review-for-seans-voice` run on the fresh draft in step 3 — its content-honesty and accuracy lenses are the fidelity check, and its other lenses catch register drift the fresh writer can introduce. For plain forms (comments, docstrings, release notes, READMEs): `/pony-prose`, plus `/pony-comments` for comments, and plain is where they stop. The detail of the three steps, the scaling, and the guardrails lives in `/writing-handoff`.

**Write plainly**: `/pony-prose` is the rulebook for how the words read in a comment, a docstring, a release note, or a README. Load it before writing any of them. For prose in Sean's voice — blog posts, LWIP, discussions, issues, PR descriptions, announcements, commit messages — `/seans-voice` holds the same plainness rules plus the carve-outs for Sean's deliberate devices, and it governs those voice forms instead. Don't load `/pony-prose` for them, even though `/pony-skills` names them; that routing index is written for people who have no `/seans-voice`.
...
**Context check: list what you are referring to, then send**: Write as if Sean has the message you're about to send and nothing else — not your context, not the files you touched, not what scrolled past. The one thing you can count on him having is the message he just sent you. So write the list before you send: every thing the draft points at — a comment, a file, a decision, a finding, a change, a point made earlier, anything you've been looking at and he hasn't — and against each one, the words in the draft that say what it *is*. Not where it lives. What it is. Quote the comment, give the sentence, state what changed; a file and a line number locate a thing, they don't say it. Naming a thing is not saying it. Any entry with nothing against it is a defect — "those two comments," "the parked item," "the fix," "it," landing before the thing itself has been stated — and what you fix is the draft, not the list. When something is too big to quote, quote the part your point rests on and say where the rest is; don't drop it, and don't send the bare reference. The list goes in your reasoning; Sean never sees it. Doing it in your head is not doing it — with nothing written down, "I checked" is a claim about a check rather than a check, and the draft will read fine to you either way, because you're holding the context that makes it read fine and he isn't. What the draft has to pass is that Sean, with this message alone, can say what you're talking about and what he's being asked. A message can be plain, short, factual, and fail that.

**Plainness check: plain in conversation, too**: When talking to Sean — explaining, proposing, summarizing — lead with the direct answer to what he asked, in the first sentence. Every sentence states a fact he can check or act on; cover the wording, and if no fact is left, cut the sentence. No coined terms, no clever phrasing, no headers or lists he didn't ask for; quoted material, and the context the context check asks for, are never a list he didn't ask for. Shorter wins — but never by cutting that context. When the two pull against each other, the message gets longer. This is the same plainness `/how-to-write` and `/seans-voice` ask for in the artifact, carried into the conversation.
...
**Commit work-in-progress as you go**: Commit after each logical chunk — don't leave edits sitting uncommitted on the branch. Uncommitted edits to tracked files are lost the moment anything switches the branch in that checkout (a background subagent, other tooling, your own mistake); only untracked files survive. Committing early keeps a branch switch from erasing work. Squash before the PR as usual.

**Don't disturb an active checkout**: Don't switch branches in — or commit into — a repo checkout parked on work that isn't yours. Before any branch or commit work, run `git rev-parse --abbrev-ref HEAD`; if it's not on `main` and not your own branch, don't switch it. Instead, clone the repo fresh from its GitHub URL (`git remote get-url origin`) into a directory under `~/tmp` and branch off `main` there — a separate clone shares no state to corrupt, and cloning from GitHub (not the local path) keeps `origin` correct so push and PR need no rewiring. Delete that clone once its work is merged, after checking it has no uncommitted or unpushed changes. Two exceptions to the clone step: in a ponyc repo, never clone (a working checkout needs the heavy vendored LLVM submodule) — stop and ask instead; and if the repo has no remote to clone from, stop, since there's nothing to clone.

**Never use git worktrees**: Don't create one, and don't pass `isolation: "worktree"` when spawning a subagent — for isolation, use a fresh clone (above).
...
11. **Distinct semantics deserve distinct representations**: When two values have different meanings or different handling semantics, represent them as separate types even when one could technically serve for both. Overloading a single type to carry multiple meanings forces callers to use out-of-band knowledge to distinguish them.

12. **It is easier to give than take away**: When deciding whether to include something in an API (a callback, a parameter, a feature), lean toward omitting it. You can always add it later if needed, but removing it is a breaking change. Start minimal; expand based on demonstrated need.

13. **Don't patch around architectural problems**: A bug report names a symptom — that's what someone saw. It isn't the scope of the fix. Find the cause, and find how far it reaches, before you decide what to change (`/pony-debug`). Then fix the cause once, where it lives: more than the symptom, so you don't guard the one call site you happened to be looking at; less than a rewrite, because knowing the cause doesn't license one. When you're done, read your fix — if you can't say why that code is there without mentioning the ticket, you fixed the ticket. If a fix requires assumptions about the internal structure of a different layer, it belongs in that layer. When the cause can't be fixed cleanly anywhere you're willing to change, say so and stop; don't write the special case. The bias toward producing a visible result ("I fixed this too!") leads to fragile code that papers over the real issue and makes the eventual proper fix harder to reason about.

14. **Trait/interface defaults only for universal invariants**: A default implementation should express behavior that's correct for every inheritor. If the correct behavior depends on which concrete type is implementing, make the method abstract at that level and push defaults down to intermediate traits or concrete types where a universal invariant actually holds. "Flip the default and add override-backs" hides state-dependent decisions behind inheritance semantics. Applies to any language with trait/interface/mixin defaults.
...
**Load `/pony-prose` and `/pony-comments` before writing a comment**: Before writing or changing any comment or docstring, load both. `/pony-prose` is the rulebook for how the words read — plainly, saying a fact the reader can check, no coined jargon, no giving a test or a bug a mind of its own. `/pony-comments` is the rulebook for what earns a comment at all, what never belongs in one, and what to do when two distant things must change together. Two things from `/pony-comments` that you will otherwise get wrong. A comment is forever, so it must never carry a fact with a shelf life: what CI runs, whether a test exists, "nothing catches this," which suite to run, or what the current body does — a report on how the code works now goes false the moment someone rewrites it. And when two far-apart things must change together, remove the coupling if you can, pin it with an assertion or a generated mirror if you can't, and comment both ends only when neither is possible.
