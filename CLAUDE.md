...
**Load `/pony-comments` before writing a comment**: Before writing or changing any comment or docstring, load `/pony-comments`. It is the rulebook — what earns a comment, what never belongs in one, and what to do when two distant things must change together. Three things from it that you will otherwise get wrong. A comment is forever, so it must never carry a fact with a shelf life: what CI runs, whether a test exists, "nothing catches this," which suite to run. Never coin a term for what the code does — no "green-skip," no private codenames; write plainly what happens. And when two far-apart things must change together, remove the coupling if you can, pin it with an assertion or a generated mirror if you can't, and comment both ends only when neither is possible.
